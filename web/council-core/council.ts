...
import { getContext } from './retriever';
import { auditResponse } from './auditor';
import { synthesize } from './synthesizer';
import { saveSession } from './session';
import { generateSearchQueries, executeSearch } from './search';
import { CouncilSession, CouncilResponse, FollowUp, ProgressEvent } from './types';
...
async function tryStage(member: any, stages: ThinkingStage[], id: string, label: string, systemPrompt: string, userPrompt: string, opts: any, format?: 'json'): Promise<string> {
  try {
    const result = await (await import('./ollama')).callOllama(systemPrompt, userPrompt, member.model, opts, format);
...
You MUST respond ONLY in valid JSON:
{
  "selectedIds": ["id1", "id2", "id3"]
    ...
    const res = await (await import('./ollama')).callOllama(
